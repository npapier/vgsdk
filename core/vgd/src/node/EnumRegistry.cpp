...
void EnumRegistry::initialize()
{
	...
	{
		vgLogDebug("(373, MIN_FILTER) already in m_toString enum registry");
		assert( false && "(373, MIN_FILTER) already in registry" );
	}
	#endif

	m_toString[ 373 ] = std::string("MIN_FILTER");
	...
	{
		vgLogDebug("(365, WRAP_T) already in m_toString enum registry");
		assert( false && "(365, WRAP_T) already in registry" );
	}
	#endif

	m_toString[ 365 ] = std::string("WRAP_T");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 369 ) != m_toString.end() )
	{
		vgLogDebug("(369, CLAMP_TO_EDGE) already in m_toString enum registry");
		assert( false && "(369, CLAMP_TO_EDGE) already in registry" );
	}
	#endif

	m_toString[ 369 ] = std::string("CLAMP_TO_EDGE");
	...
	{
		vgLogDebug("(364, WRAP_S) already in m_toString enum registry");
		assert( false && "(364, WRAP_S) already in registry" );
	}
	#endif

	m_toString[ 364 ] = std::string("WRAP_S");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 366 ) != m_toString.end() )
	{
		vgLogDebug("(366, WRAP_R) already in m_toString enum registry");
		assert( false && "(366, WRAP_R) already in registry" );
	}
	#endif

	m_toString[ 366 ] = std::string("WRAP_R");
	...
	{
		vgLogDebug("(375, NEAREST) already in m_toString enum registry");
		assert( false && "(375, NEAREST) already in registry" );
	}
	#endif

	m_toString[ 375 ] = std::string("NEAREST");
	...
	{
		vgLogDebug("(354, DEPTH_COMPONENT_32) already in m_toString enum registry");
		assert( false && "(354, DEPTH_COMPONENT_32) already in registry" );
	}
	#endif

	m_toString[ 354 ] = std::string("DEPTH_COMPONENT_32");
	...
	{
		vgLogDebug("(380, LINEAR_MIPMAP_LINEAR) already in m_toString enum registry");
		assert( false && "(380, LINEAR_MIPMAP_LINEAR) already in registry" );
	}
	#endif

	m_toString[ 380 ] = std::string("LINEAR_MIPMAP_LINEAR");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 367 ) != m_toString.end() )
	{
		vgLogDebug("(367, REPEAT) already in m_toString enum registry");
		assert( false && "(367, REPEAT) already in registry" );
	}
	#endif

	m_toString[ 367 ] = std::string("REPEAT");
	...
	{
		vgLogDebug("(360, LUMINANCE_16F) already in m_toString enum registry");
		assert( false && "(360, LUMINANCE_16F) already in registry" );
	}
	#endif

	m_toString[ 360 ] = std::string("LUMINANCE_16F");
	...
	{
		vgLogDebug("(332, INPUT1_OUTPUT_BUFFER3) already in m_toString enum registry");
		assert( false && "(332, INPUT1_OUTPUT_BUFFER3) already in registry" );
	}
	#endif

	m_toString[ 332 ] = std::string("INPUT1_OUTPUT_BUFFER3");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 368 ) != m_toString.end() )
	{
		vgLogDebug("(368, CLAMP) already in m_toString enum registry");
		assert( false && "(368, CLAMP) already in registry" );
	}
	#endif

	m_toString[ 368 ] = std::string("CLAMP");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 372 ) != m_toString.end() )
	{
		vgLogDebug("(372, ONCE) already in m_toString enum registry");
		assert( false && "(372, ONCE) already in registry" );
	}
	#endif

	m_toString[ 372 ] = std::string("ONCE");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 378 ) != m_toString.end() )
	{
		vgLogDebug("(378, LINEAR_MIPMAP_NEAREST) already in m_toString enum registry");
		assert( false && "(378, LINEAR_MIPMAP_NEAREST) already in registry" );
	}
	#endif

	m_toString[ 378 ] = std::string("LINEAR_MIPMAP_NEAREST");
	...
	{
		vgLogDebug("(353, DEPTH_COMPONENT_24) already in m_toString enum registry");
		assert( false && "(353, DEPTH_COMPONENT_24) already in registry" );
	}
	#endif

	m_toString[ 353 ] = std::string("DEPTH_COMPONENT_24");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 350 ) != m_toString.end() )
	{
		vgLogDebug("(350, SHADOW) already in m_toString enum registry");
		assert( false && "(350, SHADOW) already in registry" );
	}
	#endif

	m_toString[ 350 ] = std::string("SHADOW");
	...
	{
		vgLogDebug("(371, MIRRORED_REPEAT) already in m_toString enum registry");
		assert( false && "(371, MIRRORED_REPEAT) already in registry" );
	}
	#endif

	m_toString[ 371 ] = std::string("MIRRORED_REPEAT");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 356 ) != m_toString.end() )
	{
		vgLogDebug("(356, RGB_16F) already in m_toString enum registry");
		assert( false && "(356, RGB_16F) already in registry" );
	}
	#endif

	m_toString[ 356 ] = std::string("RGB_16F");
	...
	{
		vgLogDebug("(358, RGBA_16F) already in m_toString enum registry");
		assert( false && "(358, RGBA_16F) already in registry" );
	}
	#endif

	m_toString[ 358 ] = std::string("RGBA_16F");
	...
	{
		vgLogDebug("(349, IMAGE) already in m_toString enum registry");
		assert( false && "(349, IMAGE) already in registry" );
	}
	#endif

	m_toString[ 349 ] = std::string("IMAGE");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 352 ) != m_toString.end() )
	{
		vgLogDebug("(352, DEPTH_COMPONENT_16) already in m_toString enum registry");
		assert( false && "(352, DEPTH_COMPONENT_16) already in registry" );
	}
	#endif

	m_toString[ 352 ] = std::string("DEPTH_COMPONENT_16");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 362 ) != m_toString.end() )
	{
		vgLogDebug("(362, LUMINANCE_ALPHA_16F) already in m_toString enum registry");
		assert( false && "(362, LUMINANCE_ALPHA_16F) already in registry" );
	}
	#endif

	m_toString[ 362 ] = std::string("LUMINANCE_ALPHA_16F");
	...
	{
		vgLogDebug("(327, PREVIOUS0) already in m_toString enum registry");
		assert( false && "(327, PREVIOUS0) already in registry" );
	}
	#endif

	m_toString[ 327 ] = std::string("PREVIOUS0");
	...
	{
		vgLogDebug("(321, OUTPUT_BUFFER2) already in m_toString enum registry");
		assert( false && "(321, OUTPUT_BUFFER2) already in registry" );
	}
	#endif

	m_toString[ 321 ] = std::string("OUTPUT_BUFFER2");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 320 ) != m_toString.end() )
	{
		vgLogDebug("(320, OUTPUT_BUFFER1) already in m_toString enum registry");
		assert( false && "(320, OUTPUT_BUFFER1) already in registry" );
	}
	#endif

	m_toString[ 320 ] = std::string("OUTPUT_BUFFER1");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 319 ) != m_toString.end() )
	{
		vgLogDebug("(319, OUTPUT_BUFFER0) already in m_toString enum registry");
		assert( false && "(319, OUTPUT_BUFFER0) already in registry" );
	}
	#endif

	m_toString[ 319 ] = std::string("OUTPUT_BUFFER0");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 326 ) != m_toString.end() )
	{
		vgLogDebug("(326, OUTPUT_BUFFER7) already in m_toString enum registry");
		assert( false && "(326, OUTPUT_BUFFER7) already in registry" );
	}
	#endif

	m_toString[ 326 ] = std::string("OUTPUT_BUFFER7");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 325 ) != m_toString.end() )
	{
		vgLogDebug("(325, OUTPUT_BUFFER6) already in m_toString enum registry");
		assert( false && "(325, OUTPUT_BUFFER6) already in registry" );
	}
	#endif

	m_toString[ 325 ] = std::string("OUTPUT_BUFFER6");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 324 ) != m_toString.end() )
	{
		vgLogDebug("(324, OUTPUT_BUFFER5) already in m_toString enum registry");
		assert( false && "(324, OUTPUT_BUFFER5) already in registry" );
	}
	#endif

	m_toString[ 324 ] = std::string("OUTPUT_BUFFER5");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 323 ) != m_toString.end() )
	{
		vgLogDebug("(323, OUTPUT_BUFFER4) already in m_toString enum registry");
		assert( false && "(323, OUTPUT_BUFFER4) already in registry" );
	}
	#endif

	m_toString[ 323 ] = std::string("OUTPUT_BUFFER4");
	...
	{
		vgLogDebug("(351, AUTOMATIC) already in m_toString enum registry");
		assert( false && "(351, AUTOMATIC) already in registry" );
	}
	#endif

	m_toString[ 351 ] = std::string("AUTOMATIC");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 322 ) != m_toString.end() )
	{
		vgLogDebug("(322, OUTPUT_BUFFER3) already in m_toString enum registry");
		assert( false && "(322, OUTPUT_BUFFER3) already in registry" );
	}
	#endif

	m_toString[ 322 ] = std::string("OUTPUT_BUFFER3");
	...
	{
		vgLogDebug("(379, NEAREST_MIPMAP_LINEAR) already in m_toString enum registry");
		assert( false && "(379, NEAREST_MIPMAP_LINEAR) already in registry" );
	}
	#endif

	m_toString[ 379 ] = std::string("NEAREST_MIPMAP_LINEAR");
	...
	{
		vgLogDebug("(374, MAG_FILTER) already in m_toString enum registry");
		assert( false && "(374, MAG_FILTER) already in registry" );
	}
	#endif

	m_toString[ 374 ] = std::string("MAG_FILTER");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 338 ) != m_toString.end() )
	{
		vgLogDebug("(338, INPUT1_NONE) already in m_toString enum registry");
		assert( false && "(338, INPUT1_NONE) already in registry" );
	}
	#endif

	m_toString[ 338 ] = std::string("INPUT1_NONE");
	...
	{
		vgLogDebug("(347, INPUT2_PREVIOUS0) already in m_toString enum registry");
		assert( false && "(347, INPUT2_PREVIOUS0) already in registry" );
	}
	#endif

	m_toString[ 347 ] = std::string("INPUT2_PREVIOUS0");
	...
	{
		vgLogDebug("(331, INPUT1_OUTPUT_BUFFER2) already in m_toString enum registry");
		assert( false && "(331, INPUT1_OUTPUT_BUFFER2) already in registry" );
	}
	#endif

	m_toString[ 331 ] = std::string("INPUT1_OUTPUT_BUFFER2");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 330 ) != m_toString.end() )
	{
		vgLogDebug("(330, INPUT1_OUTPUT_BUFFER1) already in m_toString enum registry");
		assert( false && "(330, INPUT1_OUTPUT_BUFFER1) already in registry" );
	}
	#endif

	m_toString[ 330 ] = std::string("INPUT1_OUTPUT_BUFFER1");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 329 ) != m_toString.end() )
	{
		vgLogDebug("(329, INPUT1_OUTPUT_BUFFER0) already in m_toString enum registry");
		assert( false && "(329, INPUT1_OUTPUT_BUFFER0) already in registry" );
	}
	#endif

	m_toString[ 329 ] = std::string("INPUT1_OUTPUT_BUFFER0");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 336 ) != m_toString.end() )
	{
		vgLogDebug("(336, INPUT1_OUTPUT_BUFFER7) already in m_toString enum registry");
		assert( false && "(336, INPUT1_OUTPUT_BUFFER7) already in registry" );
	}
	#endif

	m_toString[ 336 ] = std::string("INPUT1_OUTPUT_BUFFER7");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 335 ) != m_toString.end() )
	{
		vgLogDebug("(335, INPUT1_OUTPUT_BUFFER6) already in m_toString enum registry");
		assert( false && "(335, INPUT1_OUTPUT_BUFFER6) already in registry" );
	}
	#endif

	m_toString[ 335 ] = std::string("INPUT1_OUTPUT_BUFFER6");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 334 ) != m_toString.end() )
	{
		vgLogDebug("(334, INPUT1_OUTPUT_BUFFER5) already in m_toString enum registry");
		assert( false && "(334, INPUT1_OUTPUT_BUFFER5) already in registry" );
	}
	#endif

	m_toString[ 334 ] = std::string("INPUT1_OUTPUT_BUFFER5");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 333 ) != m_toString.end() )
	{
		vgLogDebug("(333, INPUT1_OUTPUT_BUFFER4) already in m_toString enum registry");
		assert( false && "(333, INPUT1_OUTPUT_BUFFER4) already in registry" );
	}
	#endif

	m_toString[ 333 ] = std::string("INPUT1_OUTPUT_BUFFER4");
	...
	{
		vgLogDebug("(359, RGBA_32F) already in m_toString enum registry");
		assert( false && "(359, RGBA_32F) already in registry" );
	}
	#endif

	m_toString[ 359 ] = std::string("RGBA_32F");
	...
	{
		vgLogDebug("(328, NONE) already in m_toString enum registry");
		assert( false && "(328, NONE) already in registry" );
	}
	#endif

	m_toString[ 328 ] = std::string("NONE");
	...
	{
		vgLogDebug("(370, CLAMP_TO_BORDER) already in m_toString enum registry");
		assert( false && "(370, CLAMP_TO_BORDER) already in registry" );
	}
	#endif

	m_toString[ 370 ] = std::string("CLAMP_TO_BORDER");
	...
	{
		vgLogDebug("(339, INPUT2_OUTPUT_BUFFER0) already in m_toString enum registry");
		assert( false && "(339, INPUT2_OUTPUT_BUFFER0) already in registry" );
	}
	#endif

	m_toString[ 339 ] = std::string("INPUT2_OUTPUT_BUFFER0");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 361 ) != m_toString.end() )
	{
		vgLogDebug("(361, LUMINANCE_32F) already in m_toString enum registry");
		assert( false && "(361, LUMINANCE_32F) already in registry" );
	}
	#endif

	m_toString[ 361 ] = std::string("LUMINANCE_32F");
	...
	{
		vgLogDebug("(337, INPUT1_PREVIOUS0) already in m_toString enum registry");
		assert( false && "(337, INPUT1_PREVIOUS0) already in registry" );
	}
	#endif

	m_toString[ 337 ] = std::string("INPUT1_PREVIOUS0");
	...
	{
		vgLogDebug("(377, NEAREST_MIPMAP_NEAREST) already in m_toString enum registry");
		assert( false && "(377, NEAREST_MIPMAP_NEAREST) already in registry" );
	}
	#endif

	m_toString[ 377 ] = std::string("NEAREST_MIPMAP_NEAREST");
	...
	{
		vgLogDebug("(376, LINEAR) already in m_toString enum registry");
		assert( false && "(376, LINEAR) already in registry" );
	}
	#endif

	m_toString[ 376 ] = std::string("LINEAR");
	...
	{
		vgLogDebug("(363, LUMINANCE_ALPHA_32F) already in m_toString enum registry");
		assert( false && "(363, LUMINANCE_ALPHA_32F) already in registry" );
	}
	#endif

	m_toString[ 363 ] = std::string("LUMINANCE_ALPHA_32F");
	...
	{
		vgLogDebug("(355, DEPTH_COMPONENT_32F) already in m_toString enum registry");
		assert( false && "(355, DEPTH_COMPONENT_32F) already in registry" );
	}
	#endif

	m_toString[ 355 ] = std::string("DEPTH_COMPONENT_32F");
	...
	{
		vgLogDebug("(357, RGB_32F) already in m_toString enum registry");
		assert( false && "(357, RGB_32F) already in registry" );
	}
	#endif

	m_toString[ 357 ] = std::string("RGB_32F");
	...
	{
		vgLogDebug("(340, INPUT2_OUTPUT_BUFFER1) already in m_toString enum registry");
		assert( false && "(340, INPUT2_OUTPUT_BUFFER1) already in registry" );
	}
	#endif

	m_toString[ 340 ] = std::string("INPUT2_OUTPUT_BUFFER1");
	...
	{
		vgLogDebug("(342, INPUT2_OUTPUT_BUFFER3) already in m_toString enum registry");
		assert( false && "(342, INPUT2_OUTPUT_BUFFER3) already in registry" );
	}
	#endif

	m_toString[ 342 ] = std::string("INPUT2_OUTPUT_BUFFER3");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 341 ) != m_toString.end() )
	{
		vgLogDebug("(341, INPUT2_OUTPUT_BUFFER2) already in m_toString enum registry");
		assert( false && "(341, INPUT2_OUTPUT_BUFFER2) already in registry" );
	}
	#endif

	m_toString[ 341 ] = std::string("INPUT2_OUTPUT_BUFFER2");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 344 ) != m_toString.end() )
	{
		vgLogDebug("(344, INPUT2_OUTPUT_BUFFER5) already in m_toString enum registry");
		assert( false && "(344, INPUT2_OUTPUT_BUFFER5) already in registry" );
	}
	#endif

	m_toString[ 344 ] = std::string("INPUT2_OUTPUT_BUFFER5");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 343 ) != m_toString.end() )
	{
		vgLogDebug("(343, INPUT2_OUTPUT_BUFFER4) already in m_toString enum registry");
		assert( false && "(343, INPUT2_OUTPUT_BUFFER4) already in registry" );
	}
	#endif

	m_toString[ 343 ] = std::string("INPUT2_OUTPUT_BUFFER4");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 346 ) != m_toString.end() )
	{
		vgLogDebug("(346, INPUT2_OUTPUT_BUFFER7) already in m_toString enum registry");
		assert( false && "(346, INPUT2_OUTPUT_BUFFER7) already in registry" );
	}
	#endif

	m_toString[ 346 ] = std::string("INPUT2_OUTPUT_BUFFER7");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 345 ) != m_toString.end() )
	{
		vgLogDebug("(345, INPUT2_OUTPUT_BUFFER6) already in m_toString enum registry");
		assert( false && "(345, INPUT2_OUTPUT_BUFFER6) already in registry" );
	}
	#endif

	m_toString[ 345 ] = std::string("INPUT2_OUTPUT_BUFFER6");
	...
	{
		vgLogDebug("(348, INPUT2_NONE) already in m_toString enum registry");
		assert( false && "(348, INPUT2_NONE) already in registry" );
	}
	#endif

	m_toString[ 348 ] = std::string("INPUT2_NONE");
	...
	{
		vgLogDebug("(373, FilterParameterType) already in m_toEnum enum registry");
		assert( false && "(373, FilterParameterType) already in registry" );
	}
	#endif

	m_toEnum[ 373 ] = vgd::makeShp( new vgd::node::Texture::FilterParameterType(373) );
	...
	{
		vgLogDebug("(365, WrapParameterType) already in m_toEnum enum registry");
		assert( false && "(365, WrapParameterType) already in registry" );
	}
	#endif

	m_toEnum[ 365 ] = vgd::makeShp( new vgd::node::Texture::WrapParameterType(365) );
	...
	{
		vgLogDebug("(364, WrapParameterType) already in m_toEnum enum registry");
		assert( false && "(364, WrapParameterType) already in registry" );
	}
	#endif

	m_toEnum[ 364 ] = vgd::makeShp( new vgd::node::Texture::WrapParameterType(364) );
	...
	{
		vgLogDebug("(375, FilterValueType) already in m_toEnum enum registry");
		assert( false && "(375, FilterValueType) already in registry" );
	}
	#endif

	m_toEnum[ 375 ] = vgd::makeShp( new vgd::node::Texture::FilterValueType(375) );
	...
	{
		vgLogDebug("(367, WrapValueType) already in m_toEnum enum registry");
		assert( false && "(367, WrapValueType) already in registry" );
	}
	#endif

	m_toEnum[ 367 ] = vgd::makeShp( new vgd::node::Texture::WrapValueType(367) );
	...
	{
		vgLogDebug("(368, WrapValueType) already in m_toEnum enum registry");
		assert( false && "(368, WrapValueType) already in registry" );
	}
	#endif

	m_toEnum[ 368 ] = vgd::makeShp( new vgd::node::Texture::WrapValueType(368) );
	...
	{
		vgLogDebug("(350, UsageValueType) already in m_toEnum enum registry");
		assert( false && "(350, UsageValueType) already in registry" );
	}
	#endif

	m_toEnum[ 350 ] = vgd::makeShp( new vgd::node::Texture::UsageValueType(350) );
	...
	{
		vgLogDebug("(349, UsageValueType) already in m_toEnum enum registry");
		assert( false && "(349, UsageValueType) already in registry" );
	}
	#endif

	m_toEnum[ 349 ] = vgd::makeShp( new vgd::node::Texture::UsageValueType(349) );
	...
	{
		vgLogDebug("(374, FilterParameterType) already in m_toEnum enum registry");
		assert( false && "(374, FilterParameterType) already in registry" );
	}
	#endif

	m_toEnum[ 374 ] = vgd::makeShp( new vgd::node::Texture::FilterParameterType(374) );
	...
	{
		vgLogDebug("(376, FilterValueType) already in m_toEnum enum registry");
		assert( false && "(376, FilterValueType) already in registry" );
	}
	#endif

	m_toEnum[ 376 ] = vgd::makeShp( new vgd::node::Texture::FilterValueType(376) );
