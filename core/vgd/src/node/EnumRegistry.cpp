...
void EnumRegistry::initialize()
{
	...
	{
		vgLogDebug("(318, DOF) already in m_toString enum registry");
		assert( false && "(318, DOF) already in registry" );
	}
	#endif

	m_toString[ 318 ] = std::string("DOF");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 307 ) != m_toString.end() )
	{
		vgLogDebug("(307, BLOOM_VERT) already in m_toString enum registry");
		assert( false && "(307, BLOOM_VERT) already in registry" );
	}
	#endif

	m_toString[ 307 ] = std::string("BLOOM_VERT");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 317 ) != m_toString.end() )
	{
		vgLogDebug("(317, COMBINE3_AND_SCALE) already in m_toString enum registry");
		assert( false && "(317, COMBINE3_AND_SCALE) already in registry" );
	}
	#endif

	m_toString[ 317 ] = std::string("COMBINE3_AND_SCALE");
	...
	{
		vgLogDebug("(312, ADD) already in m_toString enum registry");
		assert( false && "(312, ADD) already in registry" );
	}
	#endif

	m_toString[ 312 ] = std::string("ADD");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 294 ) != m_toString.end() )
	{
		vgLogDebug("(294, COLOR) already in m_toString enum registry");
		assert( false && "(294, COLOR) already in registry" );
	}
	#endif

	m_toString[ 294 ] = std::string("COLOR");
	...
	{
		vgLogDebug("(286, VERY_HIGH) already in m_toString enum registry");
		assert( false && "(286, VERY_HIGH) already in registry" );
	}
	#endif

	m_toString[ 286 ] = std::string("VERY_HIGH");
	...
	{
		vgLogDebug("(263, CW) already in m_toString enum registry");
		assert( false && "(263, CW) already in registry" );
	}
	#endif

	m_toString[ 263 ] = std::string("CW");
	...
	{
		vgLogDebug("(274, SHADOW_MAPPING) already in m_toString enum registry");
		assert( false && "(274, SHADOW_MAPPING) already in registry" );
	}
	#endif

	m_toString[ 274 ] = std::string("SHADOW_MAPPING");
	...
	{
		vgLogDebug("(262, CCW) already in m_toString enum registry");
		assert( false && "(262, CCW) already in registry" );
	}
	#endif

	m_toString[ 262 ] = std::string("CCW");
	...
	{
		vgLogDebug("(273, SHADOW_OFF) already in m_toString enum registry");
		assert( false && "(273, SHADOW_OFF) already in registry" );
	}
	#endif

	m_toString[ 273 ] = std::string("SHADOW_OFF");
	...
	{
		vgLogDebug("(306, BLOOM_HORIZ) already in m_toString enum registry");
		assert( false && "(306, BLOOM_HORIZ) already in registry" );
	}
	#endif

	m_toString[ 306 ] = std::string("BLOOM_HORIZ");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 316 ) != m_toString.end() )
	{
		vgLogDebug("(316, COMBINE2_AND_SCALE) already in m_toString enum registry");
		assert( false && "(316, COMBINE2_AND_SCALE) already in registry" );
	}
	#endif

	m_toString[ 316 ] = std::string("COMBINE2_AND_SCALE");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 285 ) != m_toString.end() )
	{
		vgLogDebug("(285, HIGH) already in m_toString enum registry");
		assert( false && "(285, HIGH) already in registry" );
	}
	#endif

	m_toString[ 285 ] = std::string("HIGH");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 305 ) != m_toString.end() )
	{
		vgLogDebug("(305, BLUR_VERT) already in m_toString enum registry");
		assert( false && "(305, BLUR_VERT) already in registry" );
	}
	#endif

	m_toString[ 305 ] = std::string("BLUR_VERT");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 334 ) != m_toString.end() )
	{
		vgLogDebug("(334, INPUT1_OUTPUT_BUFFER3) already in m_toString enum registry");
		assert( false && "(334, INPUT1_OUTPUT_BUFFER3) already in registry" );
	}
	#endif

	m_toString[ 334 ] = std::string("INPUT1_OUTPUT_BUFFER3");
	...
	{
		vgLogDebug("(291, INTEGER) already in m_toString enum registry");
		assert( false && "(291, INTEGER) already in registry" );
	}
	#endif

	m_toString[ 291 ] = std::string("INTEGER");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 271 ) != m_toString.end() )
	{
		vgLogDebug("(271, AT_INFINITY) already in m_toString enum registry");
		assert( false && "(271, AT_INFINITY) already in registry" );
	}
	#endif

	m_toString[ 271 ] = std::string("AT_INFINITY");
	...
	{
		vgLogDebug("(280, SHADOW_MAPPING_32UM) already in m_toString enum registry");
		assert( false && "(280, SHADOW_MAPPING_32UM) already in registry" );
	}
	#endif

	m_toString[ 280 ] = std::string("SHADOW_MAPPING_32UM");
	...
	{
		vgLogDebug("(290, LUMINANCE_ALPHA) already in m_toString enum registry");
		assert( false && "(290, LUMINANCE_ALPHA) already in registry" );
	}
	#endif

	m_toString[ 290 ] = std::string("LUMINANCE_ALPHA");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 295 ) != m_toString.end() )
	{
		vgLogDebug("(295, NORMAL) already in m_toString enum registry");
		assert( false && "(295, NORMAL) already in registry" );
	}
	#endif

	m_toString[ 295 ] = std::string("NORMAL");
	...
	{
		vgLogDebug("(314, MIX_AND_SCALE) already in m_toString enum registry");
		assert( false && "(314, MIX_AND_SCALE) already in registry" );
	}
	#endif

	m_toString[ 314 ] = std::string("MIX_AND_SCALE");
	...
	{
		vgLogDebug("(279, SHADOW_MAPPING_32U) already in m_toString enum registry");
		assert( false && "(279, SHADOW_MAPPING_32U) already in registry" );
	}
	#endif

	m_toString[ 279 ] = std::string("SHADOW_MAPPING_32U");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 329 ) != m_toString.end() )
	{
		vgLogDebug("(329, PREVIOUS0) already in m_toString enum registry");
		assert( false && "(329, PREVIOUS0) already in registry" );
	}
	#endif

	m_toString[ 329 ] = std::string("PREVIOUS0");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 315 ) != m_toString.end() )
	{
		vgLogDebug("(315, ALPHAMIX_AND_SCALE) already in m_toString enum registry");
		assert( false && "(315, ALPHAMIX_AND_SCALE) already in registry" );
	}
	#endif

	m_toString[ 315 ] = std::string("ALPHAMIX_AND_SCALE");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 289 ) != m_toString.end() )
	{
		vgLogDebug("(289, LUMINANCE) already in m_toString enum registry");
		assert( false && "(289, LUMINANCE) already in registry" );
	}
	#endif

	m_toString[ 289 ] = std::string("LUMINANCE");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 323 ) != m_toString.end() )
	{
		vgLogDebug("(323, OUTPUT_BUFFER2) already in m_toString enum registry");
		assert( false && "(323, OUTPUT_BUFFER2) already in registry" );
	}
	#endif

	m_toString[ 323 ] = std::string("OUTPUT_BUFFER2");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 322 ) != m_toString.end() )
	{
		vgLogDebug("(322, OUTPUT_BUFFER1) already in m_toString enum registry");
		assert( false && "(322, OUTPUT_BUFFER1) already in registry" );
	}
	#endif

	m_toString[ 322 ] = std::string("OUTPUT_BUFFER1");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 321 ) != m_toString.end() )
	{
		vgLogDebug("(321, OUTPUT_BUFFER0) already in m_toString enum registry");
		assert( false && "(321, OUTPUT_BUFFER0) already in registry" );
	}
	#endif

	m_toString[ 321 ] = std::string("OUTPUT_BUFFER0");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 328 ) != m_toString.end() )
	{
		vgLogDebug("(328, OUTPUT_BUFFER7) already in m_toString enum registry");
		assert( false && "(328, OUTPUT_BUFFER7) already in registry" );
	}
	#endif

	m_toString[ 328 ] = std::string("OUTPUT_BUFFER7");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 327 ) != m_toString.end() )
	{
		vgLogDebug("(327, OUTPUT_BUFFER6) already in m_toString enum registry");
		assert( false && "(327, OUTPUT_BUFFER6) already in registry" );
	}
	#endif

	m_toString[ 327 ] = std::string("OUTPUT_BUFFER6");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 326 ) != m_toString.end() )
	{
		vgLogDebug("(326, OUTPUT_BUFFER5) already in m_toString enum registry");
		assert( false && "(326, OUTPUT_BUFFER5) already in registry" );
	}
	#endif

	m_toString[ 326 ] = std::string("OUTPUT_BUFFER5");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 325 ) != m_toString.end() )
	{
		vgLogDebug("(325, OUTPUT_BUFFER4) already in m_toString enum registry");
		assert( false && "(325, OUTPUT_BUFFER4) already in registry" );
	}
	#endif

	m_toString[ 325 ] = std::string("OUTPUT_BUFFER4");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 292 ) != m_toString.end() )
	{
		vgLogDebug("(292, FLOAT16) already in m_toString enum registry");
		assert( false && "(292, FLOAT16) already in registry" );
	}
	#endif

	m_toString[ 292 ] = std::string("FLOAT16");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 268 ) != m_toString.end() )
	{
		vgLogDebug("(268, LIGHTING_OFF) already in m_toString enum registry");
		assert( false && "(268, LIGHTING_OFF) already in registry" );
	}
	#endif

	m_toString[ 268 ] = std::string("LIGHTING_OFF");
	...
	{
		vgLogDebug("(324, OUTPUT_BUFFER3) already in m_toString enum registry");
		assert( false && "(324, OUTPUT_BUFFER3) already in registry" );
	}
	#endif

	m_toString[ 324 ] = std::string("OUTPUT_BUFFER3");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 265 ) != m_toString.end() )
	{
		vgLogDebug("(265, INT24) already in m_toString enum registry");
		assert( false && "(265, INT24) already in registry" );
	}
	#endif

	m_toString[ 265 ] = std::string("INT24");
	...
	{
		vgLogDebug("(257, ANAGLYPH) already in m_toString enum registry");
		assert( false && "(257, ANAGLYPH) already in registry" );
	}
	#endif

	m_toString[ 257 ] = std::string("ANAGLYPH");
	...
	{
		vgLogDebug("(302, COLOR_INVERSE) already in m_toString enum registry");
		assert( false && "(302, COLOR_INVERSE) already in registry" );
	}
	#endif

	m_toString[ 302 ] = std::string("COLOR_INVERSE");
	...
	{
		vgLogDebug("(340, INPUT1_NONE) already in m_toString enum registry");
		assert( false && "(340, INPUT1_NONE) already in registry" );
	}
	#endif

	m_toString[ 340 ] = std::string("INPUT1_NONE");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 267 ) != m_toString.end() )
	{
		vgLogDebug("(267, FLOAT32) already in m_toString enum registry");
		assert( false && "(267, FLOAT32) already in registry" );
	}
	#endif

	m_toString[ 267 ] = std::string("FLOAT32");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 349 ) != m_toString.end() )
	{
		vgLogDebug("(349, INPUT2_PREVIOUS0) already in m_toString enum registry");
		assert( false && "(349, INPUT2_PREVIOUS0) already in registry" );
	}
	#endif

	m_toString[ 349 ] = std::string("INPUT2_PREVIOUS0");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 281 ) != m_toString.end() )
	{
		vgLogDebug("(281, SHADOW_MAPPING_64U) already in m_toString enum registry");
		assert( false && "(281, SHADOW_MAPPING_64U) already in registry" );
	}
	#endif

	m_toString[ 281 ] = std::string("SHADOW_MAPPING_64U");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 300 ) != m_toString.end() )
	{
		vgLogDebug("(300, COLOR_TO_MONOCHROME) already in m_toString enum registry");
		assert( false && "(300, COLOR_TO_MONOCHROME) already in registry" );
	}
	#endif

	m_toString[ 300 ] = std::string("COLOR_TO_MONOCHROME");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 276 ) != m_toString.end() )
	{
		vgLogDebug("(276, SHADOW_MAPPING_4DM) already in m_toString enum registry");
		assert( false && "(276, SHADOW_MAPPING_4DM) already in registry" );
	}
	#endif

	m_toString[ 276 ] = std::string("SHADOW_MAPPING_4DM");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 333 ) != m_toString.end() )
	{
		vgLogDebug("(333, INPUT1_OUTPUT_BUFFER2) already in m_toString enum registry");
		assert( false && "(333, INPUT1_OUTPUT_BUFFER2) already in registry" );
	}
	#endif

	m_toString[ 333 ] = std::string("INPUT1_OUTPUT_BUFFER2");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 332 ) != m_toString.end() )
	{
		vgLogDebug("(332, INPUT1_OUTPUT_BUFFER1) already in m_toString enum registry");
		assert( false && "(332, INPUT1_OUTPUT_BUFFER1) already in registry" );
	}
	#endif

	m_toString[ 332 ] = std::string("INPUT1_OUTPUT_BUFFER1");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 331 ) != m_toString.end() )
	{
		vgLogDebug("(331, INPUT1_OUTPUT_BUFFER0) already in m_toString enum registry");
		assert( false && "(331, INPUT1_OUTPUT_BUFFER0) already in registry" );
	}
	#endif

	m_toString[ 331 ] = std::string("INPUT1_OUTPUT_BUFFER0");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 338 ) != m_toString.end() )
	{
		vgLogDebug("(338, INPUT1_OUTPUT_BUFFER7) already in m_toString enum registry");
		assert( false && "(338, INPUT1_OUTPUT_BUFFER7) already in registry" );
	}
	#endif

	m_toString[ 338 ] = std::string("INPUT1_OUTPUT_BUFFER7");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 337 ) != m_toString.end() )
	{
		vgLogDebug("(337, INPUT1_OUTPUT_BUFFER6) already in m_toString enum registry");
		assert( false && "(337, INPUT1_OUTPUT_BUFFER6) already in registry" );
	}
	#endif

	m_toString[ 337 ] = std::string("INPUT1_OUTPUT_BUFFER6");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 336 ) != m_toString.end() )
	{
		vgLogDebug("(336, INPUT1_OUTPUT_BUFFER5) already in m_toString enum registry");
		assert( false && "(336, INPUT1_OUTPUT_BUFFER5) already in registry" );
	}
	#endif

	m_toString[ 336 ] = std::string("INPUT1_OUTPUT_BUFFER5");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 335 ) != m_toString.end() )
	{
		vgLogDebug("(335, INPUT1_OUTPUT_BUFFER4) already in m_toString enum registry");
		assert( false && "(335, INPUT1_OUTPUT_BUFFER4) already in registry" );
	}
	#endif

	m_toString[ 335 ] = std::string("INPUT1_OUTPUT_BUFFER4");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 298 ) != m_toString.end() )
	{
		vgLogDebug("(298, LINEAR_DEPTH) already in m_toString enum registry");
		assert( false && "(298, LINEAR_DEPTH) already in registry" );
	}
	#endif

	m_toString[ 298 ] = std::string("LINEAR_DEPTH");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 287 ) != m_toString.end() )
	{
		vgLogDebug("(287, RGB) already in m_toString enum registry");
		assert( false && "(287, RGB) already in registry" );
	}
	#endif

	m_toString[ 287 ] = std::string("RGB");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 266 ) != m_toString.end() )
	{
		vgLogDebug("(266, INT32) already in m_toString enum registry");
		assert( false && "(266, INT32) already in registry" );
	}
	#endif

	m_toString[ 266 ] = std::string("INT32");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 303 ) != m_toString.end() )
	{
		vgLogDebug("(303, SCALE_AND_BIAS) already in m_toString enum registry");
		assert( false && "(303, SCALE_AND_BIAS) already in registry" );
	}
	#endif

	m_toString[ 303 ] = std::string("SCALE_AND_BIAS");
	...
	{
		vgLogDebug("(260, FRONT) already in m_toString enum registry");
		assert( false && "(260, FRONT) already in registry" );
	}
	#endif

	m_toString[ 260 ] = std::string("FRONT");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 311 ) != m_toString.end() )
	{
		vgLogDebug("(311, OVER) already in m_toString enum registry");
		assert( false && "(311, OVER) already in registry" );
	}
	#endif

	m_toString[ 311 ] = std::string("OVER");
	...
	{
		vgLogDebug("(330, NONE) already in m_toString enum registry");
		assert( false && "(330, NONE) already in registry" );
	}
	#endif

	m_toString[ 330 ] = std::string("NONE");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 288 ) != m_toString.end() )
	{
		vgLogDebug("(288, RGBA) already in m_toString enum registry");
		assert( false && "(288, RGBA) already in registry" );
	}
	#endif

	m_toString[ 288 ] = std::string("RGBA");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 299 ) != m_toString.end() )
	{
		vgLogDebug("(299, CUSTOM) already in m_toString enum registry");
		assert( false && "(299, CUSTOM) already in registry" );
	}
	#endif

	m_toString[ 299 ] = std::string("CUSTOM");
	...
	{
		vgLogDebug("(282, SHADOW_MAPPING_64UM) already in m_toString enum registry");
		assert( false && "(282, SHADOW_MAPPING_64UM) already in registry" );
	}
	#endif

	m_toString[ 282 ] = std::string("SHADOW_MAPPING_64UM");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 313 ) != m_toString.end() )
	{
		vgLogDebug("(313, SUB) already in m_toString enum registry");
		assert( false && "(313, SUB) already in registry" );
	}
	#endif

	m_toString[ 313 ] = std::string("SUB");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 297 ) != m_toString.end() )
	{
		vgLogDebug("(297, DEPTH) already in m_toString enum registry");
		assert( false && "(297, DEPTH) already in registry" );
	}
	#endif

	m_toString[ 297 ] = std::string("DEPTH");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 341 ) != m_toString.end() )
	{
		vgLogDebug("(341, INPUT2_OUTPUT_BUFFER0) already in m_toString enum registry");
		assert( false && "(341, INPUT2_OUTPUT_BUFFER0) already in registry" );
	}
	#endif

	m_toString[ 341 ] = std::string("INPUT2_OUTPUT_BUFFER0");
	...
	{
		vgLogDebug("(284, MEDIUM) already in m_toString enum registry");
		assert( false && "(284, MEDIUM) already in registry" );
	}
	#endif

	m_toString[ 284 ] = std::string("MEDIUM");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 343 ) != m_toString.end() )
	{
		vgLogDebug("(343, INPUT2_OUTPUT_BUFFER2) already in m_toString enum registry");
		assert( false && "(343, INPUT2_OUTPUT_BUFFER2) already in registry" );
	}
	#endif

	m_toString[ 343 ] = std::string("INPUT2_OUTPUT_BUFFER2");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 308 ) != m_toString.end() )
	{
		vgLogDebug("(308, COLOR_EDGE_DETECT) already in m_toString enum registry");
		assert( false && "(308, COLOR_EDGE_DETECT) already in registry" );
	}
	#endif

	m_toString[ 308 ] = std::string("COLOR_EDGE_DETECT");
	...
	{
		vgLogDebug("(304, BLUR_HORIZ) already in m_toString enum registry");
		assert( false && "(304, BLUR_HORIZ) already in registry" );
	}
	#endif

	m_toString[ 304 ] = std::string("BLUR_HORIZ");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 301 ) != m_toString.end() )
	{
		vgLogDebug("(301, COLOR_TO_SEPIA) already in m_toString enum registry");
		assert( false && "(301, COLOR_TO_SEPIA) already in registry" );
	}
	#endif

	m_toString[ 301 ] = std::string("COLOR_TO_SEPIA");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 275 ) != m_toString.end() )
	{
		vgLogDebug("(275, SHADOW_MAPPING_4U) already in m_toString enum registry");
		assert( false && "(275, SHADOW_MAPPING_4U) already in registry" );
	}
	#endif

	m_toString[ 275 ] = std::string("SHADOW_MAPPING_4U");
	...
	{
		vgLogDebug("(258, DISABLED) already in m_toString enum registry");
		assert( false && "(258, DISABLED) already in registry" );
	}
	#endif

	m_toString[ 258 ] = std::string("DISABLED");
	...
	{
		vgLogDebug("(259, BACK) already in m_toString enum registry");
		assert( false && "(259, BACK) already in registry" );
	}
	#endif

	m_toString[ 259 ] = std::string("BACK");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 256 ) != m_toString.end() )
	{
		vgLogDebug("(256, MONOSCOPIC) already in m_toString enum registry");
		assert( false && "(256, MONOSCOPIC) already in registry" );
	}
	#endif

	m_toString[ 256 ] = std::string("MONOSCOPIC");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 277 ) != m_toString.end() )
	{
		vgLogDebug("(277, SHADOW_MAPPING_16U) already in m_toString enum registry");
		assert( false && "(277, SHADOW_MAPPING_16U) already in registry" );
	}
	#endif

	m_toString[ 277 ] = std::string("SHADOW_MAPPING_16U");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 272 ) != m_toString.end() )
	{
		vgLogDebug("(272, AT_EYE) already in m_toString enum registry");
		assert( false && "(272, AT_EYE) already in registry" );
	}
	#endif

	m_toString[ 272 ] = std::string("AT_EYE");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 310 ) != m_toString.end() )
	{
		vgLogDebug("(310, UP_FILTER4) already in m_toString enum registry");
		assert( false && "(310, UP_FILTER4) already in registry" );
	}
	#endif

	m_toString[ 310 ] = std::string("UP_FILTER4");
	...
	{
		vgLogDebug("(309, DOWN_FILTER4) already in m_toString enum registry");
		assert( false && "(309, DOWN_FILTER4) already in registry" );
	}
	#endif

	m_toString[ 309 ] = std::string("DOWN_FILTER4");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 320 ) != m_toString.end() )
	{
		vgLogDebug("(320, CUSTOM_FILTER) already in m_toString enum registry");
		assert( false && "(320, CUSTOM_FILTER) already in registry" );
	}
	#endif

	m_toString[ 320 ] = std::string("CUSTOM_FILTER");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 296 ) != m_toString.end() )
	{
		vgLogDebug("(296, POSITION) already in m_toString enum registry");
		assert( false && "(296, POSITION) already in registry" );
	}
	#endif

	m_toString[ 296 ] = std::string("POSITION");
	...
	{
		vgLogDebug("(278, SHADOW_MAPPING_16UM) already in m_toString enum registry");
		assert( false && "(278, SHADOW_MAPPING_16UM) already in registry" );
	}
	#endif

	m_toString[ 278 ] = std::string("SHADOW_MAPPING_16UM");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 261 ) != m_toString.end() )
	{
		vgLogDebug("(261, FRONT_AND_BACK) already in m_toString enum registry");
		assert( false && "(261, FRONT_AND_BACK) already in registry" );
	}
	#endif

	m_toString[ 261 ] = std::string("FRONT_AND_BACK");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 342 ) != m_toString.end() )
	{
		vgLogDebug("(342, INPUT2_OUTPUT_BUFFER1) already in m_toString enum registry");
		assert( false && "(342, INPUT2_OUTPUT_BUFFER1) already in registry" );
	}
	#endif

	m_toString[ 342 ] = std::string("INPUT2_OUTPUT_BUFFER1");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 270 ) != m_toString.end() )
	{
		vgLogDebug("(270, STANDARD_PER_PIXEL) already in m_toString enum registry");
		assert( false && "(270, STANDARD_PER_PIXEL) already in registry" );
	}
	#endif

	m_toString[ 270 ] = std::string("STANDARD_PER_PIXEL");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 344 ) != m_toString.end() )
	{
		vgLogDebug("(344, INPUT2_OUTPUT_BUFFER3) already in m_toString enum registry");
		assert( false && "(344, INPUT2_OUTPUT_BUFFER3) already in registry" );
	}
	#endif

	m_toString[ 344 ] = std::string("INPUT2_OUTPUT_BUFFER3");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 339 ) != m_toString.end() )
	{
		vgLogDebug("(339, INPUT1_PREVIOUS0) already in m_toString enum registry");
		assert( false && "(339, INPUT1_PREVIOUS0) already in registry" );
	}
	#endif

	m_toString[ 339 ] = std::string("INPUT1_PREVIOUS0");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 346 ) != m_toString.end() )
	{
		vgLogDebug("(346, INPUT2_OUTPUT_BUFFER5) already in m_toString enum registry");
		assert( false && "(346, INPUT2_OUTPUT_BUFFER5) already in registry" );
	}
	#endif

	m_toString[ 346 ] = std::string("INPUT2_OUTPUT_BUFFER5");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 345 ) != m_toString.end() )
	{
		vgLogDebug("(345, INPUT2_OUTPUT_BUFFER4) already in m_toString enum registry");
		assert( false && "(345, INPUT2_OUTPUT_BUFFER4) already in registry" );
	}
	#endif

	m_toString[ 345 ] = std::string("INPUT2_OUTPUT_BUFFER4");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 348 ) != m_toString.end() )
	{
		vgLogDebug("(348, INPUT2_OUTPUT_BUFFER7) already in m_toString enum registry");
		assert( false && "(348, INPUT2_OUTPUT_BUFFER7) already in registry" );
	}
	#endif

	m_toString[ 348 ] = std::string("INPUT2_OUTPUT_BUFFER7");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 347 ) != m_toString.end() )
	{
		vgLogDebug("(347, INPUT2_OUTPUT_BUFFER6) already in m_toString enum registry");
		assert( false && "(347, INPUT2_OUTPUT_BUFFER6) already in registry" );
	}
	#endif

	m_toString[ 347 ] = std::string("INPUT2_OUTPUT_BUFFER6");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 319 ) != m_toString.end() )
	{
		vgLogDebug("(319, NO_FILTER) already in m_toString enum registry");
		assert( false && "(319, NO_FILTER) already in registry" );
	}
	#endif

	m_toString[ 319 ] = std::string("NO_FILTER");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 269 ) != m_toString.end() )
	{
		vgLogDebug("(269, STANDARD_PER_VERTEX) already in m_toString enum registry");
		assert( false && "(269, STANDARD_PER_VERTEX) already in registry" );
	}
	#endif

	m_toString[ 269 ] = std::string("STANDARD_PER_VERTEX");

	//
	#ifdef _DEBUG
	if ( m_toString.find( 350 ) != m_toString.end() )
	{
		vgLogDebug("(350, INPUT2_NONE) already in m_toString enum registry");
		assert( false && "(350, INPUT2_NONE) already in registry" );
	}
	#endif

	m_toString[ 350 ] = std::string("INPUT2_NONE");
	...
	{
		vgLogDebug("(286, ShadowQualityValueType) already in m_toEnum enum registry");
		assert( false && "(286, ShadowQualityValueType) already in registry" );
	}
	#endif

	m_toEnum[ 286 ] = vgd::makeShp( new vgd::node::LightModel::ShadowQualityValueType(286) );
	...
	{
		vgLogDebug("(263, OrientationValueType) already in m_toEnum enum registry");
		assert( false && "(263, OrientationValueType) already in registry" );
	}
	#endif

	m_toEnum[ 263 ] = vgd::makeShp( new vgd::node::FrontFace::OrientationValueType(263) );
	...
	{
		vgLogDebug("(262, OrientationValueType) already in m_toEnum enum registry");
		assert( false && "(262, OrientationValueType) already in registry" );
	}
	#endif

	m_toEnum[ 262 ] = vgd::makeShp( new vgd::node::FrontFace::OrientationValueType(262) );
	...
	{
		vgLogDebug("(285, ShadowQualityValueType) already in m_toEnum enum registry");
		assert( false && "(285, ShadowQualityValueType) already in registry" );
	}
	#endif

	m_toEnum[ 285 ] = vgd::makeShp( new vgd::node::LightModel::ShadowQualityValueType(285) );
	...
	{
		vgLogDebug("(271, ViewerValueType) already in m_toEnum enum registry");
		assert( false && "(271, ViewerValueType) already in registry" );
	}
	#endif

	m_toEnum[ 271 ] = vgd::makeShp( new vgd::node::LightModel::ViewerValueType(271) );
	...
	{
		vgLogDebug("(290, FormatValueType) already in m_toEnum enum registry");
		assert( false && "(290, FormatValueType) already in registry" );
	}
	#endif

	m_toEnum[ 290 ] = vgd::makeShp( new vgd::node::OutputBufferProperty::FormatValueType(290) );
	...
	{
		vgLogDebug("(289, FormatValueType) already in m_toEnum enum registry");
		assert( false && "(289, FormatValueType) already in registry" );
	}
	#endif

	m_toEnum[ 289 ] = vgd::makeShp( new vgd::node::OutputBufferProperty::FormatValueType(289) );
	...
	{
		vgLogDebug("(292, TypeValueType) already in m_toEnum enum registry");
		assert( false && "(292, TypeValueType) already in registry" );
	}
	#endif

	m_toEnum[ 292 ] = vgd::makeShp( new vgd::node::OutputBufferProperty::TypeValueType(292) );
	...
	m_toEnum[ 257 ] = vgd::makeShp( new vgd::node::Camera::ModeValueType(257) );
	...
	{
		vgLogDebug("(340, Input1ValueType) already in m_toEnum enum registry");
		assert( false && "(340, Input1ValueType) already in registry" );
	}
	#endif

	m_toEnum[ 340 ] = vgd::makeShp( new vgd::node::PostProcessing::Input1ValueType(340) );
	
	//
	#ifdef _DEBUG
	if ( m_toEnum.find( 267 ) != m_toEnum.end() )
	{
		vgLogDebug("(267, ShadowMapTypeValueType) already in m_toEnum enum registry");
		assert( false && "(267, ShadowMapTypeValueType) already in registry" );
	}
	#endif

	m_toEnum[ 267 ] = vgd::makeShp( new vgd::node::LightModel::ShadowMapTypeValueType(267) );
	...
	{
		vgLogDebug("(281, ShadowValueType) already in m_toEnum enum registry");
		assert( false && "(281, ShadowValueType) already in registry" );
	}
	#endif

	m_toEnum[ 281 ] = vgd::makeShp( new vgd::node::LightModel::ShadowValueType(281) );
	...
	{
		vgLogDebug("(298, CommandValueType) already in m_toEnum enum registry");
		assert( false && "(298, CommandValueType) already in registry" );
	}
	#endif

	m_toEnum[ 298 ] = vgd::makeShp( new vgd::node::OutputBufferProperty::CommandValueType(298) );
	...
	{
		vgLogDebug("(266, ShadowMapTypeValueType) already in m_toEnum enum registry");
		assert( false && "(266, ShadowMapTypeValueType) already in registry" );
	}
	#endif

	m_toEnum[ 266 ] = vgd::makeShp( new vgd::node::LightModel::ShadowMapTypeValueType(266) );
	...
	m_toEnum[ 260 ] = vgd::makeShp( new vgd::node::CullFace::ModeValueType(260) );
	...
	{
		vgLogDebug("(293, TypeValueType) already in m_toEnum enum registry");
		assert( false && "(293, TypeValueType) already in registry" );
	}
	#endif

	m_toEnum[ 293 ] = vgd::makeShp( new vgd::node::OutputBufferProperty::TypeValueType(293) );
	
	//
	#ifdef _DEBUG
	if ( m_toEnum.find( 330 ) != m_toEnum.end() )
	{
		vgLogDebug("(330, Input0ValueType) already in m_toEnum enum registry");
		assert( false && "(330, Input0ValueType) already in registry" );
	}
	#endif

	m_toEnum[ 330 ] = vgd::makeShp( new vgd::node::PostProcessing::Input0ValueType(330) );
	...
	{
		vgLogDebug("(299, CommandValueType) already in m_toEnum enum registry");
		assert( false && "(299, CommandValueType) already in registry" );
	}
	#endif

	m_toEnum[ 299 ] = vgd::makeShp( new vgd::node::OutputBufferProperty::CommandValueType(299) );
	...
	m_toEnum[ 256 ] = vgd::makeShp( new vgd::node::Camera::ModeValueType(256) );
	...
	{
		vgLogDebug("(272, ViewerValueType) already in m_toEnum enum registry");
		assert( false && "(272, ViewerValueType) already in registry" );
	}
	#endif

	m_toEnum[ 272 ] = vgd::makeShp( new vgd::node::LightModel::ViewerValueType(272) );
	...
	m_toEnum[ 261 ] = vgd::makeShp( new vgd::node::CullFace::ModeValueType(261) );
	...
	{
		vgLogDebug("(270, ModelValueType) already in m_toEnum enum registry");
		assert( false && "(270, ModelValueType) already in registry" );
	}
	#endif

	m_toEnum[ 270 ] = vgd::makeShp( new vgd::node::LightModel::ModelValueType(270) );
	...
	{
		vgLogDebug("(269, ModelValueType) already in m_toEnum enum registry");
		assert( false && "(269, ModelValueType) already in registry" );
	}
	#endif

	m_toEnum[ 269 ] = vgd::makeShp( new vgd::node::LightModel::ModelValueType(269) );
	
	//
	#ifdef _DEBUG
	if ( m_toEnum.find( 350 ) != m_toEnum.end() )
	{
		vgLogDebug("(350, Input2ValueType) already in m_toEnum enum registry");
		assert( false && "(350, Input2ValueType) already in registry" );
	}
	#endif

	m_toEnum[ 350 ] = vgd::makeShp( new vgd::node::PostProcessing::Input2ValueType(350) );
