...
:	m_arrayType( NONE )
{
	...
	const uint stride = (uint)accessor->getStride();
	switch ( stride )
	{
		case 1:
			m_arrayType = FLOAT;
	...
	const uint count = (uint)accessor->getCount();
	...
	switch( stride )
	{
		...
		case 2:
			for( size_t i = 0; i < count; ++i )
			{
				m_vec2f.push_back( vgm::Vec2f((float)va[i*2], (float)va[i*2+1]) );
			}
			break;
		...
		case 3:
			for( size_t i = 0; i < count; ++i )
			{
				m_vec3f.push_back( vgm::Vec3f((float)va[i*3], (float)va[i*3+1], (float)va[i*3+2]) );
			}
			break;
		...
		case 4:
			for( size_t i = 0; i < count; ++i )
			{
				m_vec4f.push_back( vgm::Vec4f((float)va[i*4], (float)va[i*4+1], (float)va[i*4+2], (float)va[i*4+3]) );
			}
			break;

		default:
			vgLogDebug2("Unsupported stride %i.", stride);
	}
}
